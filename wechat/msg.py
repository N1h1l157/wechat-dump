...
# 添加一些新发现的 TYPE 类型
TYPE_QUOTE_REPLY = 822083633
TYPE_MUSIC_SHARE = 1040187441
...
class WeChatMsg(object):
    ...
    def __init__(self, values):
        ...
        if self.type not in _KNOWN_TYPES:
            logger.warning("Unhandled message type: {}".format(self.type))
    ...
    def msg_str(self):
        ...
        elif self.type == TYPE_APP_MSG or self.type == TYPE_QUOTE_REPLY:
            try:
                pq = PyQuery(self.content_xml_ready, parser='xml')
                return pq('title').text()
            except:
                return "RETURN MSG: unknown"
        elif self.type == TYPE_MUSIC_SHARE:
            try:
                pq = PyQuery(self.content_xml_ready, parser='xml')
                title = pq('title').text()
                des = pq('des').text()
                return f"SHARE MUSIC: TITLE {title}, DES {des}"
            except:
                return "SHARE MUSIC: unknown"
