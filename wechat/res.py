...
class Resource(object):
    ...
    def get_voice_mp3(self, imgpath):
        ...
        if idx is None:
            return parse_wechat_audio_file(self._get_voice_filename(imgpath))
    ...
    def get_avatar(self, username):
        ...
        try:
            '''
            # 在 mac 上截图默认是包含 alpha 通道的，需要加判断
            # 如果图像是RGBA模式（包含透明通道），将其转换为RGB模式（去除透明通道）
            '''
            if im.mode == 'RGBA':
                im = im.convert('RGB')
    ...
    def _get_img_file(self, fnames):
        ...
        if len(cands) == 1:
            ...
            if name_is_thumbnail(name):
                ...
                logger.warning("Found big image but not thumbnail: {}".format(fname))
                return (name, "")
        big = cands[-1]
